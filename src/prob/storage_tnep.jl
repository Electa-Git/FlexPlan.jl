...
""
function strg_tnep(data::Dict{String,Any}, model_type::Type, solver; ref_extensions = [add_ref_dcgrid!, add_candidate_dcgrid!, add_candidate_storage!, _PM.ref_add_on_off_va_bounds!, _PM.ref_add_ne_branch!], setting = s, kwargs...)
    # if setting["process_data_internally"] == true
    #     process_additional_data!(data)
    # end
    s = setting
    return _PM.run_model(data, model_type, solver, post_strg_tnep; ref_extensions = [add_ref_dcgrid!, add_candidate_dcgrid!, add_candidate_storage!, _PM.ref_add_on_off_va_bounds!, _PM.ref_add_ne_branch!], setting = s, kwargs...)
end
