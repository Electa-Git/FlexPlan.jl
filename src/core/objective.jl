...
# distribution version: without DC equipment
function objective_min_cost_storage(pm::_PM.AbstractBFModel)
    gen_cost = Dict()
    for (n, nw_ref) in _PM.nws(pm)
        for (i,gen) in nw_ref[:gen]
            pg = _PM.var(pm, n, :pg, i)
...
# distribution version: without DC equipment
function calculate_inv_cost(pm::_PM.AbstractBFModel, n::Int)
    if haskey(pm.setting, "add_co2_cost") && pm.setting["add_co2_cost"] == true
        inv_cost = (
            sum(branch["construction_cost"]*_PM.var(pm, n, :branch_ne, i) for (i,branch) in pm.ref[:nw][n][:ne_branch])
            +
