...
function _PM.constraint_ne_ohms_yt_to(pm::LACRadPowerModel, n::Int, i, f_bus, t_bus, f_idx, t_idx, g, b, g_to, b_to, tr, ti, tm, vad_min, vad_max)
    ...
    JuMP.@constraint(pm.model, p_fr <=  M*z )
    JuMP.@constraint(pm.model, p_fr >= -M*z )
    JuMP.@constraint(pm.model, q_fr <=  M*z )
    JuMP.@constraint(pm.model, q_fr >= -M*z )
    JuMP.@constraint(pm.model, p_fr + p_to == 0 )
    JuMP.@constraint(pm.model, q_fr + q_to == 0 )
end
...
"octagonal approximation of apparent power"
function _PM.constraint_ne_thermal_limit_from(pm::LACRadPowerModel, n::Int, i, f_idx, rate_a)
    p_fr = var(pm, n, :p_ne, f_idx)
    q_fr = var(pm, n, :q_ne, f_idx)
    c_perp = 0.9238795325112867 # == cos(π/8)
    c_diag = 1.3065629648763766 # == sin(π/8) + cos(π/8) == cos(π/8) * sqrt(2)

    # p_fr == q_fr == 0 if branch is not built: enforced by constraint_ne_ohms_yt_to
    JuMP.@constraint(pm.model, -c_perp*rate_a <= p_fr        <= c_perp*rate_a)
    JuMP.@constraint(pm.model, -c_perp*rate_a <=        q_fr <= c_perp*rate_a)
    JuMP.@constraint(pm.model, -c_diag*rate_a <= p_fr + q_fr <= c_diag*rate_a)
    JuMP.@constraint(pm.model, -c_diag*rate_a <= p_fr - q_fr <= c_diag*rate_a)
...
"nothing to do, no line losses in this model"
function _PM.constraint_ne_thermal_limit_to(pm::LACRadPowerModel, n::Int, i, t_idx, rate_a)
end
