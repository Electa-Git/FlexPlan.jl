function read_demand_data(year; mc = false)
    ...
    if mc == false
        ...
        demand_north_pu = convert(Matrix, CSV.read(join(["./test/data/MC_scenarios/35_yearly_clusters/case_6_demand_","$year",".csv"])))[:,3]
        demand_center_north_pu = convert(Matrix, CSV.read(join(["./test/data/MC_scenarios/35_yearly_clusters/case_6_demand_","$year",".csv"])))[:,2]
        demand_center_south_pu = convert(Matrix, CSV.read(join(["./test/data/MC_scenarios/35_yearly_clusters/case_6_demand_","$year",".csv"])))[:,4]
        demand_south_pu = convert(Matrix, CSV.read(join(["./test/data/MC_scenarios/35_yearly_clusters/case_6_demand_","$year",".csv"])))[:,5]
        demand_sardinia_pu = convert(Matrix, CSV.read(join(["./test/data/MC_scenarios/35_yearly_clusters/case_6_demand_","$year",".csv"])))[:,6]
    end
