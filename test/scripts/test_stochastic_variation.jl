...
for p1 in 0:0.1:1
    ...
    plot_filename = join(["./test/data/output_files/lec","$idx",".kml"])
    ...
    lp_file = join(["./test/data/output_files/profile_","$idx",".json"])
    Plots.savefig(p,lp_file)


    for ld in 1:5
        p = _FP.plot_flex_demand(result, ld, data, extradata)
        lp_result = join(["./test/data/output_files/load_porfile_","$ld","_","$idx",".json"])
        Plots.savefig(p,lp_result)
    end

    res[1, idx] = result["objective"]

    result_file = join(["./test/data/output_files/stoch_result_","$idx",".json"])
