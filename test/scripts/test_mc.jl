...
gurobi = JuMP.with_optimizer(Gurobi.Optimizer, MIPGap = 0.05, Method = 3) #, TimeLimit = 10
cplex = JuMP.with_optimizer(CPLEX.Optimizer, CPX_PARAM_EPGAP=0.05)
...
for series_number in 0:5
        number_of_hours = 720 # Number of time points
        monte_carlo_generation = true
        file = "./test/data/case6_realistic_costs_v1_noflex.m"  #Input case, in matpower m-file format: Here 6bus case with candidate AC, DC lines and candidate storage
        ...
        scenario["sc_years"]["1"]["length"] = "monthly" 
        scenario["sc_years"]["1"]["n_clusters"] = 6
        scenario["sc_years"]["1"]["method"] = "_pca"
        ...
        #p = _FP.plot_profile_data(extradata, number_of_hours, Dict() ,["3" "5" "6"])
        result = _FP.flex_tnep(mn_data, _PM.DCPPowerModel, gurobi, multinetwork=true; setting = s)



        series_number = scenario["sc_years"]["1"]["series_number"] 
        ts_length = scenario["sc_years"]["1"]["length"]
        n_clusters = scenario["sc_years"]["1"]["n_clusters"]
        method = scenario["sc_years"]["1"]["method"]
        

        result_file = join(["/Users/hergun/Box Sync/Projects/FlexPlan/WP1/Code/testing/mc_tests/","$n_clusters","_",ts_length,"_clusters",method,"/","$series_number","_v5.json"])
